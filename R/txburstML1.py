...
def finalanalysis(file):
    rpkm = pd.read_csv(file, index_col=0)
    params = Parallel(n_jobs=3, verbose=3)(
        delayed(MaximumLikelihood)(np.around(rpkm[pd.notnull(rpkm)])) for i, rpkm in rpkm.iterrows())
    keep = whichKeep(params)
    pra = pd.DataFrame([ params, list(keep)], columns=rpkm.index).T
    pra.to_csv("E:\\DB\\Dropbox\\Qinfei\\Python\\Splatter\\Xin_bursting_output.csv")




# finalanalysis(file="E:\\DB\\Dropbox\\Qinfei\\Python\\Splatter\\Xin_bursting.csv")